...
class SiaLPRelaxedPJADMM(SiaILP):
  def __init__(self, num_nodes, ngpus_per_node, policy_options, solver_options):
    ...
    self.solver_bin_lambda = solver_options.pop('bin_lambda', 0)
  ...
  def solve(self):
    ...
    min_rho = self.solver_viol_beta * (self.solver_target_num_blocks / (2 - self.solver_dual_tau) - 1) * (jnp.max(A_singular_vals[0])**2) / 5
    ...
    for i in range(self.solver_max_iters):
      ...
      if (i+1) % self.solver_iters_per_sync == 0:
        ...
        if previous_iter_vals is None:
          ...
          rprint(f"\tGPU Duals: {gpu_duals_k.round(3)}")
